import os
os.environ["PROTOCOL_BUFFERS_PYTHON_IMPLEMENTATION"] = "python"

import pandas as pd
import numpy as np
import matplotlib.pyplot as plt
import tensorflow as tf
from tensorflow.keras.models import Sequential
from tensorflow.keras.layers import LSTM, Dense, Dropout, Input
from tensorflow.keras.callbacks import EarlyStopping
from sklearn.preprocessing import MinMaxScaler
from sklearn.metrics import mean_absolute_error, mean_absolute_percentage_error
import warnings

warnings.filterwarnings("ignore")

# -------------------------------------------
# 1. Función para cargar y agregar datos
# -------------------------------------------
def load_and_aggregate(data_path, use_log_transform=False):
    # Cargar datos
    df = pd.read_csv(data_path)
    df["Order Date"] = pd.to_datetime(df["Order Date"], errors="coerce")
    df.set_index("Order Date", inplace=True)
    # Agregar a nivel semanal
    weekly_sales = df["Sales"].resample("W").sum().fillna(0)
    # Opcional: aplicar logaritmo para estabilizar la varianza
    if use_log_transform:
        weekly_sales = np.log1p(weekly_sales)  # log1p para evitar problemas con ceros
    return weekly_sales

# -------------------------------------------
# 2. Función para crear secuencias
# -------------------------------------------
def create_sequences(data, window_size):
    """
    data: arreglo 2D, de forma (n_samples, n_features)
    window_size: número de períodos en cada secuencia
    Retorna X (secuencias) y y (valor a predecir)
    """
    X, y = [], []
    for i in range(len(data) - window_size):
        X.append(data[i:i+window_size])
        y.append(data[i+window_size])
    return np.array(X), np.array(y)

# -------------------------------------------
# 3. Definir y construir el modelo LSTM
# -------------------------------------------
def build_lstm_model(input_shape, dropout_rate=0.2):
    model = Sequential()
    model.add(Input(shape=input_shape))
    model.add(LSTM(50, return_sequences=True))
    model.add(Dropout(dropout_rate))
    model.add(LSTM(50))
    model.add(Dropout(dropout_rate))
    model.add(Dense(1))
    model.compile(optimizer='adam', loss='mean_squared_error')
    return model

# -------------------------------------------
# 4. Función de evaluación
# -------------------------------------------
def evaluate_model(y_true, y_pred):
    mae = mean_absolute_error(y_true, y_pred)
    mape = mean_absolute_percentage_error(y_true, y_pred) * 100
    return mae, mape

# -------------------------------------------
# 5. Función main() con mejoras
# -------------------------------------------
def main():
    # Parámetros
    data_path = "../data/processed/walmart_cleaned.csv"  # Ajusta la ruta
    use_log = True        # Activar la transformación logarítmica
    window_size = 4      # Número de semanas por secuencia
    epochs = 50
    batch_size = 16

    # Cargar y agregar datos
    weekly_sales = load_and_aggregate(data_path, use_log_transform=use_log)
    
    # Dividir en entrenamiento (80%) y test (20%)
    split_index = int(len(weekly_sales) * 0.8)
    train_series = weekly_sales.iloc[:split_index]
    test_series = weekly_sales.iloc[split_index:]
    
    # Normalizar la serie
    scaler = MinMaxScaler(feature_range=(0, 1))
    train_scaled = scaler.fit_transform(train_series.values.reshape(-1, 1))
    test_scaled = scaler.transform(test_series.values.reshape(-1, 1))
    
    # Crear secuencias
    X_train, y_train = create_sequences(train_scaled, window_size)
    X_test, y_test = create_sequences(test_scaled, window_size)
    
    # Ajustar la forma para LSTM: (n_samples, window_size, n_features)
    X_train = X_train.reshape((X_train.shape[0], X_train.shape[1], 1))
    X_test = X_test.reshape((X_test.shape[0], X_test.shape[1], 1))
    
    input_shape = (X_train.shape[1], X_train.shape[2])
    
    # Construir el modelo LSTM
    model = build_lstm_model(input_shape)
    
    # Callbacks: EarlyStopping para evitar sobreentrenamiento
    early_stop = EarlyStopping(monitor='val_loss', patience=10, restore_best_weights=True)
    
    # Entrenar el modelo
    history = model.fit(X_train, y_train, epochs=epochs, batch_size=batch_size, 
                        validation_split=0.1, verbose=1, callbacks=[early_stop])
    
    # Predicciones
    train_pred_scaled = model.predict(X_train)
    test_pred_scaled = model.predict(X_test)
    
    # Volver a la escala original
    train_pred = scaler.inverse_transform(train_pred_scaled)
    y_train_orig = scaler.inverse_transform(y_train)
    test_pred = scaler.inverse_transform(test_pred_scaled)
    y_test_orig = scaler.inverse_transform(y_test)
    
    # Si se aplicó la transformación logarítmica, revertirla con expm1
    if use_log:
        train_pred = np.expm1(train_pred)
        y_train_orig = np.expm1(y_train_orig)
        test_pred = np.expm1(test_pred)
        y_test_orig = np.expm1(y_test_orig)
        # También revertir en las series para graficar
        train_series = np.expm1(train_series)
        test_series = np.expm1(test_series)
    
    # Evaluar el modelo
    mae_test, mape_test = evaluate_model(y_test_orig, test_pred)
    print(f"Test MAE: {mae_test:.2f}, Test MAPE: {mape_test:.2f}%")
    
    # Crear tabla de resultados (recordar que al crear secuencias, la primera fecha corresponde a window_size)
    test_dates = test_series.index[window_size:]
    df_results = pd.DataFrame({
        "Fecha": test_dates,
        "Valor Real": y_test_orig.flatten(),
        "Valor Predicho": test_pred.flatten()
    })
    print("\nTabla de Resultados (primeras 10 filas):")
    print(df_results.head(10))
    
    # Graficar la serie original, la de entrenamiento, test y la predicción
    plt.figure(figsize=(12,6))
    plt.plot(train_series.index, train_series.values, label="Train", color="blue")
    plt.plot(test_series.index, test_series.values, label="Test", color="black")
    plt.plot(df_results["Fecha"], df_results["Valor Predicho"], label="LSTM Predicción", color="red", linestyle="--")
    plt.xlabel("Fecha")
    plt.ylabel("Ventas Semanales")
    plt.title("Predicción con LSTM")
    plt.legend()
    plt.grid(True)
    plt.show()
    
    # Graficar el error absoluto a lo largo del tiempo
    plt.figure(figsize=(12,6))
    plt.plot(df_results["Fecha"], np.abs(df_results["Valor Real"] - df_results["Valor Predicho"]), label="Error Absoluto", color="magenta")
    plt.xlabel("Fecha")
    plt.ylabel("Error Absoluto")
    plt.title("Error Absoluto en Test")
    plt.legend()
    plt.grid(True)
    plt.show()

if __name__ == "__main__":
    main()
